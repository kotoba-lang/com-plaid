(ns plaid.main
  "Kotodama WASM entrypoint for the Plaid clean-room actor (L4) — Clojure port.
  ...
  facts (`plaid.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Account"         :plural "accounts"         :id-prefix "plaid_acc"
    :fields [:accountId :name :officialName :mask :type :subtype :verificationStatus]
    :required [:name :officialName]
    :coerce {}
    :refs {:accountId "Account"}}
   {:entity "AccountBalance"  :plural "accountbalances"  :id-prefix "plaid_acc"
    :fields [:available :current :limit :isoCurrencyCode :lastUpdatedDatetime]
    :required [:available :current]
    :coerce {:available :float :current :float :limit :float}
    :refs {}}
   {:entity "Transaction"     :plural "transactions"     :id-prefix "plaid_tra"
    :fields [:accountId :transactionId :amount :date :pending :name :merchantName :isoCurrencyCode :paymentChannel :checkNumber]
    :required [:amount :date]
    :coerce {:amount :float :pending :bool}
    :refs {:accountId "Account" :transactionId "Transaction"}}
   {:entity "Item"            :plural "items"            :id-prefix "plaid_ite"
    :fields [:itemId :institutionId :institutionName :webhook :authMethod :oauth]
    :required [:institutionName :webhook]
    :coerce {:oauth :bool}
    :refs {:itemId "Item" :institutionId "Institution"}}
   {:entity "Institution"     :plural "institutions"     :id-prefix "plaid_ins"
    :fields [:institutionId :name :url :oauth]
    :required [:name :url]
    :coerce {:oauth :bool}
    :refs {:institutionId "Institution"}}
   {:entity "Holding"         :plural "holdings"         :id-prefix "plaid_hol"
    :fields [:accountId :securityId :symbol :name :quantity :institutionValue :costBasis :currencyCode]
    :required [:symbol :name]
    :coerce {:quantity :float :institutionValue :float :costBasis :float}
    :refs {:accountId "Account"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"plaid.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "plaid-compat" :tier tier :entities entities} 200])
